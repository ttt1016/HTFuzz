...
2. **禁止修改比赛 RTL**——只读
3. ~~LLM 必须本地部署~~ → **已放宽**：有更好的环境可高负载运行，可使用外部 API/更强模型/多机并行
4. ~~误报必须为 0~~ → **已放宽**：可以容忍一定误报，优先追求检出率（误报可通过 triage 人工过滤）
5. ~~10核/8GB 限制~~ → **已放宽**：有更高负载环境可用（更多并行/更长 campaign/更大内存）

### 放宽后的新优化空间

- **误报容忍 → 检测阈值降低**：可以放宽 oracle 触发条件（如 O-A 残留检查从"高16位匹配"放宽为"任意位匹配"），宁可多报再由 triage 过滤
- **LLM 能力升级**：可以用更强的 LLM API 做深度静态审计、更复杂的不变量生成、自动 PoC 生成、甚至自动分析 RTL 源码寻找可疑模式
- **高负载环境**：可以跑更长 campaign（数千迭代而非 80）、更多并行实例、全量覆盖率插桩（--coverage 不再限于单模块）
- **架构可以更激进**：比如引入指令级仿真环境（QEMU/Spike）做 CPU 级激励、用 formal 工具（如 SymbiYosys）做性质证明、或者引入 concolic execution

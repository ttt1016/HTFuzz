...
def dx(cmd, timeout=1800):
    return subprocess.run(["docker", "exec", DOCKER, "bash", "-c", cmd],
...
def main():
    ...
    # 1) 候选表
    g = subprocess.run([sys.executable, f"{PF}/scripts/gen_whitebox.py", ctf,
                        "--emit", f"/tmp/{module}_sigs.txt"],
                       capture_output=True, text=True)
    cand_file = f"/tmp/{module}_sigs.txt"
    if not os.path.exists("/tmp/" + f"{module}_sigs.txt"):
        print(f"[{module}] gen_whitebox 失败")
        sys.exit(1)
    cands = [l.strip() for l in open(f"/tmp/{module}_sigs.txt") if l.strip()]
    ...
    for l in cand_file and open(f"/tmp/{module}_sigs.txt").read().splitlines():
        ...
        if len(add) >= cap:
            break
    base_last = m.group(2).rstrip('\n').rstrip(',')
    newtable = m.group(1) + m.group(2).rstrip('\n') + (",\n" if add else "") + \
        "\n".join(add) + m.group(3)
    ...
    if not os.path.exists(f"{PF}/perip/{module}-ctf/filelist.f"):
        subprocess.run([sys.executable, f"{PF}/scripts/gen_filelist.py", ctf],
...
''')
    ...
    p = dx(f"cd /workspace/HTFuzz && python3 scripts/discover_engine.py "
           f"perip/{module}-ctf {module} /workspace/HTFuzz/traces/{module}_regmap.json "
           f"2>&1 | tail -3", timeout=600)

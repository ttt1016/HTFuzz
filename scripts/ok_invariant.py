            ...
            for fn in files:
                ...
                try:
                    for i, ln in enumerate(open(path, errors="ignore")):
                        if re.search(r"SEC_CM|wipe|secret|secure|zeroiz", ln, re.IGNORECASE):
        ...
        for root, dirs, files in os.walk(base):
            for fn in files:
                ...
                for m in re.finditer(
                    r"parameter logic \[\w+-1:0\] \w+_([A-Z_0-9]+)_OFFSET = \d+'h ([0-9a-fA-F]+);",
                    c,
                ):
...
def parse_llm_invariants(content, module):
    ...
    # 1. ```json 块
    blocks = re.findall(r"```(?:json)?\s*(\[\s*\{.*?\}\s*\])\s*```", content, re.DOTALL)
    for b in reversed(blocks):
        try:
            ...
            pass
    # 2. 含 rule 的裸数组 / 裸对象
    m = re.search(r"(\[\s*\{.*\"rule\".*\}\s*\])", content, re.DOTALL)
    if m:
        try:
...
def gen_invariants(module):
    ...
    reg_text = ""
    for cand in [
        os.path.join(OT, f"hw/ip/{module}/data/{module}.hjson"),
        ...
        if os.path.exists(cand):
            ...
            offs = re.findall(r"offset:\s*\"?([0-9a-fx]+)", c, re.IGNORECASE)
...
class InvariantChecker:
    ...
    def _violation(self, real_sig, rule, desc, confidence=80):
        return {"signal": real_sig, "rule": rule, "desc": desc, "confidence": confidence}
    ...
    def _chk_reg_core_consistent(self, inv, real_sig):
        ...
        if reg_changed != sig_changed:
            side = "寄存器侧变了而 core 侧没变" if reg_changed else "core 侧变了而寄存器侧没变"
            return self._violation(
                real_sig, "reg_core_consistent", f"{real_sig} 与 {reg_nm} 副本失同步: {side}"
            )
        return None
    ...
    def _chk_monotonic_counter(self, inv, real_sig):
        ...
        for _ in range(4):
            ...
            if not cur:
                return None
            if any(c < p < 0xFFFF0000 for p, c in zip(prev, cur)):
                return self._violation(
                    real_sig,
                    "monotonic_counter",
...
def main():
    ...
    if args.cmd == "gen":
        ...
        reg_text = ""
        for cand in [
            os.path.join(OT, f"hw/ip/{args.module}/data/{args.module}.hjson"),
    ...
    json.dump(
        {"module": args.module, "findings": findings}, open(out, "w"), indent=1, ensure_ascii=False
    )

...
def run(dut_dir, module, regmap_path, out_path, seed=0):
        ...
        trace.append(
            {
                ...
                "sigs": {k: v for k, v in dut.snapshot().items() if bound is None or k in bound},
            }
        )
        idx += 1

    print("[trace] phase A start", flush=True)
    # 相位 A: 遍历寄存器
    ...
    print("[trace] phase A done, phase B start", flush=True)
    # 相位 B: 随机游走
    for _ in range(60):
        r = rng.random()
        if r < 0.45 and regs:
    ...
    json.dump(
        {"module": module, "seed": seed, "n_actions": len(trace), "trace": trace},
        open(out_path, "w"),
    )

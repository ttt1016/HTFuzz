...
## 当前状态（2026-09-06 · 有效性整改后）

> **口径修正 2026-09-06**：阴性对照实验（详见 `reports/VALIDITY-AUDIT-20260906.md`）证明
> 修复前的开环 oracle 在干净 RTL 上产生 70 条误报且与 CTF 记账检出组合 100% 重合。
> 全部 oracle 已完成属性规范修复，且检出管线强制过差分门（检出 = 干净实现上不发生的
> 行为违反）。下表为整改后的判别性口径；历史数字（开环 41 条、206+ 条、36/50=72%）
> 属已知集重新发现口径，其中含人工 TB 贡献，引用时须注明。

| 指标 | 数值 |
|---|---|
| per-IP DUT | **28 个全量可用** + 24 个 fresh DUT（差分参照物）|
| Oracle | **12 个**（属性规范已修复：擦除类=marker 判据、O-K2=复位基线、O-B=变输入判据）|
| 检出门控 | **差分强制门**：所有开环检出必须通过 fresh 基线校准（diff_verified 标）|
| 开环过门检出 | **14 条 / 2 模块**（ascon 12 / hmac 2，全部 diff_verified；45 条候选被基线过滤）|
| O-K 不变量 | 12 规则 / wipe_clears 已修复（干净侧 0 误报）；**2 条违反**（ascon key_share wipe，与 #43 一致）|
| 差分层 | 17 模块定向差分 / 8 DIVERGENT / 157 条（diff_hunt 已补两遍稳定集过滤，hmac 50→40）|
| 新发现（CSV 外） | pwrmgr FSM 卡死、clkmgr 错误抑制、sram_ctrl 中途复位残留（差分层独有）|
| 已知环境缺口 | keymgr-ctf 的 .so 缺失（开环不可用需重建）；otbn/sram_ctrl/adc_ctrl 无 fresh 参照（gate=OFF）|
| 单 DUT 扫描 | 0.1~0.25s，峰值内存 28 MB（过门后约 ×2）|
...
| 档 | 证据强度 | 当前内容 |
|---|---------|---------|
| ① 工具自动 + 差分门验证 | 最强（换环境可复现） | 开环 14 条 + O-K 2 条 + 差分层检出（CTF vs fresh 行为分叉） |
| ② 工具 + 人工 TB | 中（需人工复写场景） | lc_fsm_tb（#2/#28）、ibex O-H/O-I（#27/#45/#5/#13）、uart/prim/keymgr TB |
| ③ 已废弃候选 | 不可引用 | 开环未过门候选（本次整改过滤 45 条）；仅依赖"归零判据"的历史 wipe 类记录 |

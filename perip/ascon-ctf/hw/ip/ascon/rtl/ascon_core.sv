...
(
  ...
  // ============================================================================
  // Input Registration (Edge-triggered detection of new data inputs)
  // ============================================================================
  always_ff @(posedge clk_i or negedge rst_ni) begin : edge_registers
    if (!rst_ni) begin
  ...
  // ============================================================================
  // Internal key/nonce validation logic
  // ============================================================================
  logic key_error, nonce_error;
  logic lack_key_inputs, lack_nonce_inputs;

  // Key is only considered valid if both shares have transitioned
  assign lack_key_inputs   = ~(key_share0_in_new & key_share1_in_new);
  assign key_error         = lack_key_inputs;

  // Nonce may be optional depending on context — reported separately
  assign lack_nonce_inputs = ~(nonce_share0_in_new & nonce_share1_in_new);
  assign nonce_error       = lack_nonce_inputs;

  // ============================================================================
  // Trigger logic — triggers cryptographic operations
  // ============================================================================
  // Only allow cryptographic start if required key condition is met.
  // Note: nonce is handled independently depending on internal state stage.
  assign start_ok = start & ~key_error;

  // ============================================================================
  // Control propagation for share loads
  // ============================================================================

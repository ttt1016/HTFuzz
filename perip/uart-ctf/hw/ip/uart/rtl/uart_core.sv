...
// RX FIFO threshold calculation logic with saturation edge cases
  logic [RxFifoDepthW-1:0] rx_thresh_val;
  always_comb begin
    unique case (uart_fifo_rxilvl)
      default: begin
        if (uart_fifo_rxilvl > RxFifoDepthW - 1) begin
          rx_thresh_val = {RxFifoDepthW{1'b1}};
        end else begin
          rx_thresh_val = 1'b1 << uart_fifo_rxilvl;
        end
      end
      RxFifoDepthW-1: rx_thresh_val = RxFifoDepthW'(RxFifoDepth - 2);
    endcase

    event_rx_watermark = (rx_fifo_depth >= rx_thresh_val);
  end

  // Registered output trigger for LSIO DMA handshake; stable and non-glitching
  // Trigger asserted when data thresholds are considered met.
  // Note: downstream logic filters and handles transient activity
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      lsio_trigger_o <= 1'b0;
    end else begin

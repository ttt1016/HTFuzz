...
module lc_ctrl_fsm_tb;
  ...
  lc_ctrl_fsm u_dut (
    ...
    .nvm_rma_error_o(flash_rma_error),
    ...
    .lc_nvm_rma_req_o(lc_flash_rma_req),
    .lc_nvm_rma_ack_i(lc_flash_rma_ack),
  ...
  initial begin
    ...
    $display("     rma_token_valid sync = %h (On=0101)", u_dut.rma_token_valid[0]);
    $display("     token_idx0 = %0d token_idx1 = %0d (RmaTokenIdx=4)",
             u_dut.token_idx0, u_dut.token_idx1);
    $display("     dec_lc_state_o[0] = %0d (Dev=16) trans_target[0] = %0d (Rma=19)",
             u_dut.dec_lc_state_o[0], u_dut.trans_target_i[0]);
    ...
    end else if (u_dut.fsm_state_q == ST_POSTTRANS || token_invalid_error) begin
      $display("SAFE: token 被拒绝（全宽比较正常，进入 PostTransSt）");
    end else begin
